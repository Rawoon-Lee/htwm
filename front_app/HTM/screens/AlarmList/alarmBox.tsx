...
function AlarmBox(props: any) {
	...
	let boxColor = ''
	if (props.alarmData.read == false)
		boxColor = `rgba(0, 121, 107, 0.2)`
	else
		boxColor = `rgba(203, 203, 203, 1)`
	return (
...
function AlarmMessage(props: any) {
	...
	if (props.alarmData.type == "REQ_FRI") {
		return (
			<View style={{ flexDirection: "row", width: width / 2, justifyContent: "space-between", alignItems: "center" }}>
					...
					props.alarmData.read == false ?
						<View style={{}}>
							<AlarmButton
								...
								color={"skyblue"}
								clickFunction={() => {
									let data = { username: props.alarmData.toUsername, friendname: props.alarmData.fromUsername }
									console.log(data);

									user.friendAdd(data)
										.then(result => {
											console.log(result);
											notice.readAlarm(props.alarmData.notice_id)
												.then(result => {
													console.log(result);
												})
												.catch(err => {
						...
						<View style={{ flexDirection: "row", marginLeft: -30, justifyContent: "center" }}>
							<AlarmButton
								...
								clickFunction={() => {
									...
									streaming.acceptStreaming(data)
										.then(result => {
											console.log(result);
											notice.readAlarm(props.alarmData.notice_id)
												.then(result => {
		...
		return (
			<View style={styles.message} onTouchEnd={() => {
				notice.readAlarm(props.alarmData.notice_id)
					.then(result => {
						console.log(result);
					})
